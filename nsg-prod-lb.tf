...
resource "oci_core_network_security_group_security_rule" "nsg_prod_lb_ingress_https_gmp" {
...
resource "oci_core_network_security_group_security_rule" "nsg_prod_lb_ingress_http_gmp" {
    for_each                  = local.ips.gmp_domains
    network_security_group_id = oci_core_network_security_group.nsg_prod_lb.id
    direction                 = "INGRESS"
    protocol                  = "6"
...
resource "oci_core_network_security_group_security_rule" "nsg_prod_lb_ingress_https_v1" {
...
resource "oci_core_network_security_group_security_rule" "nsg_prod_lb_ingress_http_v1" {
    for_each                  = local.ips.v1_domains
    network_security_group_id = oci_core_network_security_group.nsg_prod_lb.id
    direction                 = "INGRESS"
    protocol                  = "6"
...
module "nsg_prod_lb_egress_all_out" {
    source                 = "./modules/network-sec-rules"
    net_sec_group_id       = oci_core_network_security_group.nsg_prod_lb.id
